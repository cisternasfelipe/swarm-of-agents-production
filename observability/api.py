...
def _build_loop_summary(run_id: str) -> list[dict]:
    events = repo.get_loop_events(run_id)
    loops: dict[int, dict] = {}
    for e in events:
...
def list_runs(
    ...
    return repo.list_runs(limit=limit, status=status, before=before)
...
def get_events(
    ...
    if not repo.run_exists(run_id):
        _not_found(run_id)
    return repo.get_events(run_id, after_seq=after_seq, limit=limit)
...
async def ws_run(websocket: WebSocket, run_id: str, after_seq: int = 0):
    ...
    try:
        ...
        while True:
            events = repo.get_events(run_id, after_seq=after_seq, limit=100)
            if not events:
                break
        ...
        final_events = repo.get_events(run_id, after_seq=after_seq, limit=100)
        if final_events:
